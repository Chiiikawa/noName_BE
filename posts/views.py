...
class LikeView(APIView):
    ...
    def post(self, request, post_id):
        ...
        if existing_like:
            existing_like.delete() # 사용자와 post간 like instance가 있다면, post request가 들어오면 like instance 삭제
            return Response(
...
class CommentView(APIView):
    ...
    def post(self, request, post_id): # 댓글 생성
        ...
        return Response(
            {"detail": "유효하지 않은 댓글","errors": serializer.errors}, status=status.HTTP_400_BAD_REQUEST
            )
...
class DalleAPIView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        user = request.user
        if user.point < API_REQUEST_POINT:
            return Response({"detail": "포인트가 부족합니다."}, status=status.HTTP_403_FORBIDDEN)
        user.point = F("point") - API_REQUEST_POINT
        user.save()
        user.refresh_from_db() # Django의 query expression F()를 사용한 후에는 refresh_from_db() 사용해서 DB의 값을 갱신해줘야 함.
